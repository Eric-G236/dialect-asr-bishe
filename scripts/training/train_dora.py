#!/usr/bin/env python3
"""Standalone DoRA fine-tuning script.

Dedicated training script for the DoRA experiment.  It uses the same single-GPU
configuration as the LoRA runs so the two methods stay directly comparable:

* ``--batch-size 48000`` (token-based dynamic batching)
* ``--num-workers 8``
* fp32 by default (``--no-use-fp16``)
* LoRA/DoRA adapters on encoder and decoder ``q,k,v,o``

DoRA replaces FunASR's LoRA layers via ``dora_train_entry.py`` before the model
is built.  Logging, loss curves, checkpointing, and final evaluation follow the
same contract as the other experiments.
"""
...
def main() -> int:
    ...
    parser.add_argument("--run-name", default="dora_bs48000_ep20")
    parser.add_argument(
        "--output-root",
        type=Path,
    ...
    packaged_model = fft.package_model(
        ...
        avg_nbest_model=args.avg_nbest_model,
        adapter_kwargs={"lora_cfg": lora_cfg, "dora": True},
    )
