...
Dedicated training script for the LoRA experiment.  It bakes in a single-GPU
configuration tuned to use as much RTX 4090D memory as possible:

* ``--batch-size 48000`` (token-based dynamic batching)
* ``--num-workers 8``
* fp32 by default (``--no-use-fp16``)

LoRA adapters are attached to both encoder and decoder (``q,k,v,o``) with the
same rank/alpha/dropout used throughout the project.  Logging, real-time loss
curves, checkpointing, and final evaluation follow the same contract as the
other experiments: ``train_lora.log``, ``loss_history.json``,
``loss_curve.png``, and ``*.errors.tsv`` with the overall CER in the first row.
"""
...
def main() -> int:
    ...
    parser.add_argument("--run-name", default="lora_bs48000_ep20")
    parser.add_argument(
        "--output-root",
        type=Path,
    ...
    parser.add_argument("--batch-size", type=int, default=48000)
    ...
    packaged_model = fft.package_model(
        ...
        avg_nbest_model=args.avg_nbest_model,
        adapter_kwargs={"lora_cfg": lora_cfg, "dora": False},
    )
