#!/usr/bin/env python3
"""Evaluate a pretrained Paraformer ASR model on dev/test manifests.

Outputs, for each split:
  - <split>.predictions.jsonl : model outputs
  - <split>.errors.tsv        : per-utterance error table
        ...
        def flush_batch() -> None:
            ...
            for idx, row in enumerate(batch_rows):
                ...
                if not hyp and idx < len(results):
                    hyp = results[idx].get("text", "") if isinstance(results[idx], dict) else ""
